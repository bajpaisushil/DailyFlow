...
/** How often a reminder comes back. See `@/lib/repeat`. */
export type RepeatKind = 'weekly' | 'monthly' | 'yearly' | 'once'
...
export interface Automation extends BaseRecord {
  ...
  window?: {
    ...
    until: LocalDate
    /**
     * Exact dates to fire on, instead of every day between `from` and `until`.
     *
     * This is how monthly and yearly reminders are expressed: they cannot be a range walk,
     * because the gap between occurrences is longer than any horizon worth walking.
     */
    dates?: LocalDate[]
  }
  /**
...
export interface Reminder extends BaseRecord {
  ...
  /**
   * How often this comes back, chosen by the KIND of thing it is.
   *
   * Absent means 'weekly' — the days-of-week model every existing reminder uses, so nothing
